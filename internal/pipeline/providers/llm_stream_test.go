...
func TestParseStreamChunk_midStreamError(t *testing.T) {
	...
	_, _, _, _, _, err := parseStreamChunk(payload)
	if err == nil {
...
func TestParseStreamChunk_contentDelta(t *testing.T) {
	...
	text, _, _, _, activity, err := parseStreamChunk(payload)
	if err != nil {
		t.Fatal(err)
	}
	if text != "Hello" {
	...
	if activity {
		t.Fatal("content delta should not count as idle activity")
	}
}

func TestParseStreamChunk_reasoningActivity(t *testing.T) {
	payload := `{"choices":[{"index":0,"delta":{"content":"","reasoning_content":"...thinking..."},"finish_reason":null}]}`
	text, _, _, _, activity, err := parseStreamChunk(payload)
	if err != nil {
		t.Fatal(err)
	}
